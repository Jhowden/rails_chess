...
describe MoveValidations::Validations do
  ...
  describe "#empty_space?" do
    it "checks to see if a position is empty" do
      ...
      expect( subject.empty_space?( 2, 6 ) ).to be_truthy
      expect( subject.empty_space?( 1, 0 ) ).to be_falsey
    end
  end
  ...
  describe "#different_team?" do
    it "checks to see if an encountered piece is of the same team" do
      subject.chess_board[0][1] = piece
      expect( subject.different_team?( 1, 0, piece2 ) ).to be_falsey
      expect( subject.different_team?( 1, 0, piece3 ) ).to be_truthy
    end
  end
  ...
  describe "#valid_location?" do
    it "checks to see if a location is valid" do
      subject.chess_board[0][1] = piece
      expect( subject.valid_location?( 1, 0, piece2 ) ).to be_falsey
      expect( subject.valid_location?( 1, 0, piece3 ) ).to be_truthy
      expect( subject.valid_location?( 2, 5, piece ) ).to be_truthy
    end
  end
end